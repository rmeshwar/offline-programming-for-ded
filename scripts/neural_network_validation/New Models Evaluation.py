import os
import sys
import pandas as pd
import numpy as np
from sklearn.preprocessing import StandardScaler
from tensorflow.keras.models import load_model
import joblib
import traceback

# File paths
NEW_SAMPLES_PATH = 'data/New_DED.xlsx'  # New data samples

# Models paths
MODEL_PATHS = {
    "OLS Ridge": {
        "model": 'saved_models/OLS_ridge/neural_network_model.keras',
        "scaler": 'saved_models/OLS_ridge/scaler.pkl'
    },
    "OLS Ridge NODROPOUT": {
        "model": 'saved_models/OLS_ridge_NODROPOUT/neural_network_model.keras',
        "scaler": 'saved_models/OLS_ridge_NODROPOUT/scaler.pkl'
    },
    "DOE Ridge": {
        "model": 'saved_models/DOE_Ridge/neural_network_model.keras',
        "scaler": 'saved_models/DOE_Ridge/scaler.pkl'
    },
    "DOE Ridge NODROPOUT": {
        "model": 'saved_models/DOE_Ridge_NODROPOUT/neural_network_model.keras',
        "scaler": 'saved_models/DOE_Ridge_NODROPOUT/scaler.pkl'
    }

}

# Output log file
os.makedirs('logs/comparison', exist_ok=True)
OUTPUT_LOG = open('logs/comparison/new_models_comparison.txt', 'w', encoding='utf-8')

# Helper class for logging to both console and file
class DualOutput:
    def __init__(self, file1, file2):
        self.file1 = file1
        self.file2 = file2

    def write(self, data):
        self.file1.write(data)
        self.file2.write(data)

    def flush(self):
        pass

sys.stdout = DualOutput(sys.stdout, OUTPUT_LOG)

# Evaluation function
def evaluate_model_on_new_samples(model_path, scaler_path):
    print(f"\nEvaluating model: {model_path}")
    print(f"Using dataset: Original 32 Samples")
    print(f"Data file: {NEW_SAMPLES_PATH}\n{'-'*50}")

    # Load data
    data = pd.read_excel(NEW_SAMPLES_PATH)
    X = data[['Travel Speed', 'Wire Feed Speed', 'Stepover Distance']]
    y = data[['Ratio of Valley Area to Bead Area',
              'Ratio of Bead Heights']]

    # Load model and scaler
    model = load_model(model_path)
    scaler = joblib.load(scaler_path)

    # Scale data
    X_scaled = scaler.transform(X)

    # Predict
    predictions = model.predict(X_scaled)

    # Slice predictions to match the target variables
    predictions = predictions[:, :2]

    # Calculate Mean Squared Error (MSE) and Mean Absolute Error (MAE) for each target variable
    mse = np.mean((predictions - y.values)**2, axis=0)
    mae = np.mean(np.abs(predictions - y.values), axis=0)

    # Output comparison
    comparison_df = pd.DataFrame({
        'Actual Valley Area': y['Ratio of Valley Area to Bead Area'],
        'Predicted Valley Area': predictions[:, 0],
        'Actual Bead Heights': y['Ratio of Bead Heights'],
        'Predicted Bead Heights': predictions[:, 1],
    })

    print("Sample Predictions:")
    print(comparison_df.head(), "\n")
    print(f"MSE for each variable: {mse}")
    print(f"MAE for each variable: {mae}")
    return mse, mae, comparison_df

# Compare models on the new data
results = {}
comparison_dfs = {}

for model_name, paths in MODEL_PATHS.items():
    try:
        mse, mae, comparison_df = evaluate_model_on_new_samples(paths['model'], paths['scaler'])
        results[model_name] = {'MSE': mse, 'MAE': mae}
        comparison_dfs[model_name] = comparison_df
    except Exception as e:
        print(f"Error evaluating {model_name}: {traceback.format_exc()}")

# Final results
print("\nComparison Results Summary:")
comparison_summary = pd.DataFrame(results).T
print(comparison_summary.to_string())

# Save comparison results to CSV
comparison_summary.to_csv('logs/comparison/new_models_summary.csv')
for model_name, df in comparison_dfs.items():
    df.to_csv(f'logs/comparison/{model_name}_predictions.csv', index=False)

OUTPUT_LOG.close()
